                    ...
                    <tr class="hover:bg-gray-50/50 transition">
                        ...
                        <td class="px-6 py-4 text-center">
                            @if($item->status == 'Unpaid')
                                <span class="px-3 py-1 bg-red-100 text-red-600 rounded-full text-[10px] font-black uppercase">Belum Bayar</span>
                            @elseif($item->status == 'Paid')
                                <span class="px-3 py-1 bg-blue-100 text-blue-600 rounded-full text-[10px] font-black uppercase">Lunas (Menunggu)</span>
                            @elseif($item->status == 'Disewa')
                                <span class="px-3 py-1 bg-orange-100 text-orange-600 rounded-full text-[10px] font-black uppercase">Sedang Disewa</span>
                        ...
                        <td class="px-6 py-4 text-center">
                            ...
                            @if($item->status == 'Paid')
                                <form action="{{ route('admin.penyewaan.updateStatus', $item->id) }}" method="POST">
                                    ...
                                    <button type="submit" class="bg-[#0f172a] hover:bg-gray-800 text-white px-4 py-1.5 rounded-lg text-xs font-bold transition">
                                        Serahkan Alat
                                    </button>
                                </form>
                            @elseif($item->status == 'Disewa')
                                <form action="{{ route('admin.penyewaan.updateStatus', $item->id) }}" method="POST">
                                    ...
                                    <button type="submit" class="bg-[#f3a933] hover:bg-[#d98e1d] text-[#0f172a] px-4 py-1.5 rounded-lg text-xs font-bold transition">
                                        Barang Kembali
                                    </button>
                                </form>
                            @else
