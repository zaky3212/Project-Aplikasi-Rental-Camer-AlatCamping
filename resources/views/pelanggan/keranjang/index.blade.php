            ...
            <div class="w-full lg:w-2/3 space-y-4">
                ...
                <div class="bg-white rounded-2xl p-4 md:p-5 shadow-sm border border-gray-100 flex flex-col sm:flex-row items-start sm:items-center gap-5 hover:shadow-md transition">
                    ...
                    <div class="flex-grow w-full">
                        ...
                        <!-- Kontrol Durasi Sewa -->
                        <div class="flex items-center gap-3 bg-gray-50 w-fit p-1 rounded-lg border border-gray-200">
                            <button class="w-8 h-8 flex items-center justify-center rounded-md bg-white text-gray-600 hover:bg-gray-200 hover:text-gray-900 transition shadow-sm">
                ...
                <div class="bg-[#0f172a] rounded-2xl p-6 md:p-8 shadow-xl sticky top-28 text-white border border-white/5 relative overflow-hidden">
                    ...
                    <div class="space-y-4 text-sm text-gray-300 mb-6 border-b border-white/10 pb-6 relative z-10">
                        <div class="flex justify-between items-center">
                            <span>Subtotal ({{ $keranjang->count() }} Alat)</span>
                            <span class="font-medium">Rp {{ number_format($subtotal, 0, ',', '.') }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span>Biaya Layanan</span>
                            <span class="font-medium text-emerald-400">Gratis</span>
                        </div>
                    </div>

                    <div class="flex justify-between items-end mb-8 relative z-10">
                        <span class="text-xs uppercase tracking-widest font-bold text-gray-400">Total Pembayaran</span>
                        <span class="text-2xl font-black text-[#f3a933]">Rp {{ number_format($subtotal, 0, ',', '.') }}</span>
                    </div>

                    <form action="#" method="POST" class="relative z-10">
                        @csrf
                        <button type="submit" class="w-full py-4 bg-[#f3a933] text-[#0f172a] rounded-xl text-xs font-black uppercase tracking-widest hover:bg-[#d98e1d] transition shadow-[0_10px_20px_rgba(243,169,51,0.3)] flex justify-center items-center gap-2" {{ $keranjang->isEmpty() ? 'disabled' : '' }}>
